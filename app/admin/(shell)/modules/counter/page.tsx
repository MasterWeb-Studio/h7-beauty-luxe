...
async function ModuleListContainer({ spec }: { spec: any }) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_APP_URL}/api/admin/modules/counter?pageSize=50`,
    { cache: 'no-store' }
  );
  ...
  return (
    <ModuleList
      ...
      items={items}
      total={total}
      newHref="/admin/modules/counter/new"
      editHrefFn={(item: any) => `/admin/modules/counter/${item.id}`}
