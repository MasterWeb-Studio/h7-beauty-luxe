...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setRecordId(id);
      const res = await fetch(`/api/admin/modules/contact-cards/${id}`);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: spec.meta.displayName.tr ?? spec.meta.id, href: '/admin/modules/contact-cards' },
          { label: 'Düzenle' },
        ]}
      />
      <ModuleDetail
        ...
        autoSaveDraft={true}
        onSave={async (values) => {
          const res = await fetch(`/api/admin/modules/contact-cards/${recordId}`, {
            method: 'PATCH',
        ...
        onDelete={async (id) => {
          await fetch(`/api/admin/modules/contact-cards/${id}`, { method: 'DELETE' });
          router.push('/admin/modules/contact-cards');
        }}
        onCancel={() => router.push('/admin/modules/contact-cards')}
      />
    </div>
  );
}
