...
function fieldsFromSpec(spec: any) {
  return spec.fields.map((f: any) => ({
    name: f.name,
    type: f.type === 'jsonb' ? 'textarea' : f.type,
...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setRecordId(id);
      const res = await fetch(`/api/admin/modules/projects/${id}`);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: spec.meta.displayName.tr ?? spec.meta.id, href: '/admin/modules/projects' },
          { label: 'Düzenle' },
        ]}
      />
      <ModuleDetail
        ...
        onSave={async (values) => {
          const res = await fetch(`/api/admin/modules/projects/${recordId}`, {
            method: 'PATCH',
        ...
        onDelete={async (id) => {
          await fetch(`/api/admin/modules/projects/${id}`, { method: 'DELETE' });
          router.push('/admin/modules/projects');
        }}
        onCancel={() => router.push('/admin/modules/projects')}
      />
    </div>
  );
}
