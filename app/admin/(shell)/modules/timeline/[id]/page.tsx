...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  const spec = getSpecById('timeline')!;
  ...
  useEffect(() => {
    (async () => {
      ...
      setRecordId(id);
      const res = await fetch(`/api/admin/modules/timeline/${id}`);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: spec.meta.displayName.tr ?? spec.meta.id, href: '/admin/modules/timeline' },
          { label: 'Düzenle' },
        ]}
      />
      <ModuleDetail
        ...
        showCharCount={true}
        onSave={async (values) => {
          const res = await fetch(`/api/admin/modules/timeline/${recordId}`, {
            method: 'PATCH',
        ...
        onDelete={async (id) => {
          await fetch(`/api/admin/modules/timeline/${id}`, { method: 'DELETE' });
          router.push('/admin/modules/timeline');
        }}
        onCancel={() => router.push('/admin/modules/timeline')}
      />
    </div>
  );
}
