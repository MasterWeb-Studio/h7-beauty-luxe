...
async function ModuleListContainer({ spec }: { spec: any }) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_APP_URL}/api/admin/modules/timeline?pageSize=50`,
    { cache: 'no-store' }
  );
  ...
  return (
    <ModuleList
      ...
      items={items}
      total={total}
      newHref="/admin/modules/timeline/new"
      editHrefFn={(item: any) => `/admin/modules/timeline/${item.id}`}
