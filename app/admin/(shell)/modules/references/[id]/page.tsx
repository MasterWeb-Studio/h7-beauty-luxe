...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setRecordId(id);
      const res = await fetch(`/api/admin/modules/references/${id}`);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: spec.meta.displayName.tr ?? spec.meta.id, href: '/admin/modules/references' },
          { label: 'Düzenle' },
        ]}
      />
      <ModuleDetail
        ...
        showCharCount={true}
        onSave={async (values) => {
          const res = await fetch(`/api/admin/modules/references/${recordId}`, {
            method: 'PATCH',
        ...
        onDelete={async (id) => {
          await fetch(`/api/admin/modules/references/${id}`, { method: 'DELETE' });
          router.push('/admin/modules/references');
        }}
        onCancel={() => router.push('/admin/modules/references')}
      />
    </div>
  );
}
