...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  const spec = getSpecById('gallery')!;
  ...
  useEffect(() => {
    (async () => {
      ...
      setRecordId(id);
      const res = await fetch(`/api/admin/modules/gallery/${id}`);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: spec.meta.displayName.tr ?? spec.meta.id, href: '/admin/modules/gallery' },
          { label: 'Düzenle' },
        ]}
      />
      <ModuleDetail
        ...
        onSave={async (values) => {
          const res = await fetch(`/api/admin/modules/gallery/${recordId}`, {
            method: 'PATCH',
        ...
        onDelete={async (id) => {
          await fetch(`/api/admin/modules/gallery/${id}`, { method: 'DELETE' });
          router.push('/admin/modules/gallery');
        }}
        onCancel={() => router.push('/admin/modules/gallery')}
        previewEnabled={spec.admin.detailView.previewEnabled}
        autoSaveDraft={spec.admin.detailView.autoSaveDraft}
        showCharCount={spec.admin.detailView.showCharCount}
      />
    </div>
  );
}
