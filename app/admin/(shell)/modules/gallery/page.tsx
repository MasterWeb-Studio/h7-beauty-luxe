...
async function ModuleListContainer({ spec }: { spec: any }) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_APP_URL}/api/admin/modules/gallery?pageSize=50`,
    { cache: 'no-store' }
  );
  ...
  return (
    <ModuleList
      ...
      items={items}
      total={total}
      newHref="/admin/modules/gallery/new"
      editHrefFn={(item: any) => `/admin/modules/gallery/${item.id}`}
