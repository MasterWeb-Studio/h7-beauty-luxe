...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  const spec = getSpecById('newsletter')!;
  ...
  useEffect(() => {
    (async () => {
      ...
      setRecordId(id);
      const res = await fetch(`/api/admin/modules/newsletter/${id}`);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: spec.meta.displayName.tr ?? spec.meta.id, href: '/admin/modules/newsletter' },
          { label: 'Düzenle' },
        ]}
      />
      <ModuleDetail
        ...
        onSave={async (values) => {
          const res = await fetch(`/api/admin/modules/newsletter/${recordId}`, {
            method: 'PATCH',
        ...
        onDelete={async (id) => {
          await fetch(`/api/admin/modules/newsletter/${id}`, { method: 'DELETE' });
          router.push('/admin/modules/newsletter');
        }}
        onCancel={() => router.push('/admin/modules/newsletter')}
      />
    </div>
  );
}
