...
async function ModuleListContainer({ spec }: { spec: any }) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_APP_URL}/api/admin/modules/newsletter?pageSize=100`,
    { cache: 'no-store' }
  );
  ...
  return (
    <ModuleList
      ...
      items={items}
      total={total}
      newHref="/admin/modules/newsletter/new"
      editHrefFn={(item: any) => `/admin/modules/newsletter/${item.id}`}
