...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  const spec = getSpecById('services')!;
  ...
  useEffect(() => {
    (async () => {
      ...
      setRecordId(id);
      const res = await fetch(`/api/admin/modules/services/${id}`);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: spec.meta.displayName.tr ?? spec.meta.id, href: '/admin/modules/services' },
          { label: 'Düzenle' },
        ]}
      />
      <ModuleDetail
        ...
        onSave={async (values) => {
          const res = await fetch(`/api/admin/modules/services/${recordId}`, {
            method: 'PATCH',
        ...
        onDelete={async (id) => {
          await fetch(`/api/admin/modules/services/${id}`, { method: 'DELETE' });
          router.push('/admin/modules/services');
        }}
        onCancel={() => router.push('/admin/modules/services')}
        autoSaveDraft={true}
        showCharCount={true}
        previewEnabled={true}
      />
