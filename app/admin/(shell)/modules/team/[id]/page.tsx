...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setRecordId(id);
      const res = await fetch(`/api/admin/modules/team/${id}`);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: spec.meta.displayName.tr ?? spec.meta.id, href: '/admin/modules/team' },
          { label: 'Düzenle' },
        ]}
      />
      <ModuleDetail
        ...
        onSave={async (values) => {
          const res = await fetch(`/api/admin/modules/team/${recordId}`, {
            method: 'PATCH',
        ...
        onDelete={async (id) => {
          await fetch(`/api/admin/modules/team/${id}`, { method: 'DELETE' });
          router.push('/admin/modules/team');
        }}
        onCancel={() => router.push('/admin/modules/team')}
        autoSaveDraft={true}
        showCharCount={true}
        previewEnabled={true}
      />
