...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setRecordId(id);
      const res = await fetch(`/api/admin/modules/products/${id}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <ModuleDetail
        ...
        onSave={async (values) => {
          const res = await fetch(`/api/admin/modules/products/${recordId}`, {
            method: 'PATCH',
        ...
        onDelete={async (id) => {
          await fetch(`/api/admin/modules/products/${id}`, { method: 'DELETE' });
          router.push('/admin/modules/products');
        }}
